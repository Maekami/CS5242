...
def step2_comparison(llm,tokenizer,sampling_params,temp_data,output_path):
    ...
    return temp2_data
def step3_analyze(llm,tokenizer,sampling_params,temp_data,output_path,data_type,analyze_type):
    prompts = [
        construct_teacher_prompt(item["matched_question"], flatten_cot(item["matched_cot"]), item["test_question"], item["comparison_idea"], data_type,analyze_type).strip()
        for item in temp_data
    ]  
