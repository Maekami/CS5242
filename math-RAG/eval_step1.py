...
def teacher_OT(args):
    ...
    original_refine_data = generate_prompt_best_of_n(
    llm=llm,
    tokenizer=tokenizer,
    ...
    # Making iterative refinement
    final_data = iterative_refinement(model=llm, tokenizer=tokenizer, input_data=original_refine_data, args=args, sampling_params=sampling_params, iterations=3)
    output_path = os.path.join(args.teacher_cot_path, f"generated_reasonings_{args.data_name}_final.jsonl") # TODO: the content is the same as final iteration
